...
export function calculatePriceChange(
  ...
  const changeRate = open !== 0 ? (changeAmount / open) * 100 : 0;
  const isRise = close >= open;
  const changeColor = isRise ? upColor : downColor;

  // 포맷된 문자열 생성
  const sign = isRise ? '+' : '';
  const formattedAmount = Math.abs(changeAmount).toLocaleString();
  const formattedRateNum = Math.abs(changeRate).toFixed(2);
