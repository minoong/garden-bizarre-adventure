...
import { DEFAULT_CANDLE_COUNT, BITHUMB_ENDPOINTS } from '../model/constants';
...
export async function fetchMinuteCandles(unit: MinuteUnit, params: MinuteCandlesParams): Promise<MinuteCandle[]> {
  try {
    ...
    console.log('[API Debug] Minute candles response length:', data.length);
    return data;
  } catch (e) {
    console.error('[API Debug] Minute candles fetch failed:', e);
    throw e;
  }
}
...
export async function fetchDayCandles(params: DayCandlesParams): Promise<DayCandle[]> {
  const { data } = await bithumbClient.get<DayCandle[]>(BITHUMB_ENDPOINTS.CANDLES_DAYS, {
    params: {
      market: params.market,
      to: params.to,
      count: params.count ?? DEFAULT_CANDLE_COUNT,
...
export async function fetchWeekCandles(params: PeriodCandlesParams): Promise<WeekCandle[]> {
  const { data } = await bithumbClient.get<WeekCandle[]>(BITHUMB_ENDPOINTS.CANDLES_WEEKS, {
    params: {
      market: params.market,
      to: params.to,
      count: params.count ?? DEFAULT_CANDLE_COUNT,
...
export async function fetchMonthCandles(params: PeriodCandlesParams): Promise<MonthCandle[]> {
  const { data } = await bithumbClient.get<MonthCandle[]>(BITHUMB_ENDPOINTS.CANDLES_MONTHS, {
    params: {
      market: params.market,
      to: params.to,
      count: params.count ?? DEFAULT_CANDLE_COUNT,
