...
export interface WebSocketTrade {
  ...
  /** 변화 금액 (부호 없는 전일 대비 값) */
  change_price: number;
  /** 최근 거래 일자 (KST, yyyy-MM-dd) */
  trade_date: string;
  /** 최근 거래 시각 (KST, HH:mm:ss) */
  trade_time: string;
  /** 체결 타임스탬프 (ms) */
  trade_timestamp: number;
  /** 체결 번호 (Unique) */
...
/** WebSocket Trade 요청 */
export interface WebSocketTradeRequest {
  type: 'trade';
  codes: string[];
  isOnlySnapshot?: boolean;
  isOnlyRealtime?: boolean;
...
/** WebSocket 요청 메시지 타입 */
export type WebSocketRequest = WebSocketTickerRequest | WebSocketOrderbookRequest | WebSocketTradeRequest;

/** WebSocket 응답 메시지 타입 */
export type WebSocketResponse = WebSocketTicker | WebSocketOrderbook | WebSocketTrade;
