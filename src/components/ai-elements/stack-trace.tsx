...
export const StackTrace = memo(({ trace, className, open, defaultOpen = false, onOpenChange, onFilePathClick, children, ...props }: StackTraceProps) => {
  ...
  return (
    <StackTraceContext.Provider value={contextValue}>
      <div className={cn('not-prose bg-background w-full overflow-hidden rounded-lg border font-mono text-sm', className)} {...props}>
        {children}
      </div>
  ...
  return (
    <Collapsible onOpenChange={setIsOpen} open={isOpen}>
      <CollapsibleTrigger asChild {...props}>
        <div className={cn('hover:bg-muted/50 flex w-full cursor-pointer items-center gap-3 p-3 text-left transition-colors', className)}>{children}</div>
      </CollapsibleTrigger>
    </Collapsible>
  );
});
...
export const StackTraceExpandButton = memo(({ className, ...props }: StackTraceExpandButtonProps) => {
  ...
  return (
    <div className={cn('flex size-7 items-center justify-center', className)} {...props}>
      <ChevronDownIcon className={cn('text-muted-foreground size-4 transition-transform', isOpen ? 'rotate-180' : 'rotate-0')} />
    </div>
  );
});
...
const FilePathButton = memo(({ frame, onFilePathClick }: FilePathButtonProps) => {
  ...
  return (
    <button
      className={cn('hover:text-primary underline decoration-dotted', onFilePathClick && 'cursor-pointer')}
