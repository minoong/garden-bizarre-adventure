  ...
  if (hasChildren) {
    return (
      <Collapsible defaultOpen={depth < 2}>
        <CollapsibleTrigger
          className={cn('group hover:bg-muted/50 flex w-full items-center gap-2 py-3 text-left transition-colors', className)}
          style={{ paddingLeft }}
        >
          <ChevronRightIcon className="text-muted-foreground size-4 shrink-0 transition-transform group-data-[state=open]:rotate-90" />
          <span className="font-mono text-sm">{name}</span>
        ...
        {description && (
          <p className="text-muted-foreground pb-2 text-sm" style={{ paddingLeft: paddingLeft + 24 }}>
            {description}
          </p>
        )}
        <CollapsibleContent>
...
export const SchemaDisplayExample = ({ className, children, ...props }: SchemaDisplayExampleProps) => (
  <pre className={cn('bg-muted mx-4 mb-4 overflow-auto rounded-md p-4 font-mono text-sm', className)} {...props}>
    {children}
  </pre>
);
